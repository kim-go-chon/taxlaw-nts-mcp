...
- 연계 시 `search_law`의 `mst`/`lawId`를 `get_law_text`에 전달하고, NTS `DOC_ID`를 법령 식별자로 사용하지 않습니다. 동일 제명·MST·시점·조문을 이미 확인했다면 결과를 재사용합니다.
- 판례 보완은 `search_decisions(domain="precedent")`, 후속 인용 검사는 `legal_analysis(mode="cite_check")`입니다. 연혁 도구는 `legal_research(task="amendment_track")` 또는 `discover_tools`로 확인합니다. 서버 버전별 도구 목록을 확인하고 과거 별칭을 추측해 호출하지 않습니다.
- 두 출처의 사건번호는 공백·하이픈을 정규화하고 일자·제목을 함께 대조하여 중복을 제거하되 양쪽 출처 ID/URL을 보존합니다. NTS 실존 검증은 문서번호·회신번호의 전체 번호가 일치할 때만 확정하며, 제목 속 참조 번호·번호 접두 일치는 근거가 아닙니다. 실존 확인은 명제의 법적 타당성을 보증하지 않습니다.
