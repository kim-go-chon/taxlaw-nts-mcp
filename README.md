...
## 이 도구가 왜 좋은가 — 비개발자용 요약

**한 줄 요약** — 국세청 「국세법령정보시스템」을 매번 사이트에서 검색·복붙하지 않고, AI에게 물으면 세법해석례·기본통칙·심판례까지 찾아 **출처와 함께** 정리해 주는 도구입니다.

**비유** — 법제처(국가법령정보센터)가 *법 조문·판례 원본 도서관*이라면, 국세법령정보시스템은 *세무 실무 해석 전용 서고*(세법해석례·질의회신·기본통칙·홈택스 상담사례)입니다. 이 MCP는 그 서고에 상주하는 사서로, 원문을 꺼내오고 "이 해석이 아직 유효한지"까지 확인해 줍니다.

### 세법 리서치할 때 구체적으로 달라지는 점

1. **법제처엔 안 뜨는 자료를 찾는다** — 세법해석례·질의회신·기본통칙·홈택스 상담사례·조세심판례·발간책자는 법 조문 DB에는 없고 이 서고에만 있습니다. "법은 알겠는데 국세청이 실제로 어떻게 보느냐"를 바로 확인합니다.
2. **없는 해석례를 지어내는 걸 막는다 (★ 가장 중요)** — AI가 그럴듯한 "국심2019서1234" 같은 번호를 만들어내는 게 최대 위험입니다. 이 도구는 답변에 넣기 전 실제 DB에 그 번호가 있는지 대조(`verify_nts_citations`)하고, 없는 인용은 걸러내거나 "공개DB 미발견"으로만 표기합니다.
3. **옛날·폐지된 해석을 현행처럼 쓰는 사고를 막는다** — 귀속연도로 물으면 그 해에 적용되던 조문 버전으로 답하고(`build_application_timetable`), 인용할 해석례가 아직 유효한지·사문화·폐지됐는지 자동 판정하며(`assess_doctrine_validity`), 조문 신·구 버전을 나란히 대조(`diff_article_versions`)합니다.
4. **계산식이 든 조문을 통째로 가져온다** — 조특법 고용증대·통합고용 세액공제처럼 표·수식이 든 조문은 다른 법령 API가 수식을 누락하기 쉽습니다. 이 도구는 원문(수식 포함) 그대로 가져와 공제액 계산이 틀어지는 것을 방지합니다.
5. **업종코드 감면 판정을 자동화한다** — "이 업종코드가 조특법 감면 대상이냐"를 표준산업분류(KSIC)와 매핑해 해당/제외/범위밖/애매로 판정(`classify_industry_for_article`)합니다. 창업중소기업·중소기업특별세액감면에서 업종 오분류를 줄입니다.
6. **답이 실무 문서 형태로 정리된다** — 줄글이 아니라 결론 → 케이스별 매트릭스(표) → 법령 근거(출처별 분리·원문링크) → AI 보충(⚠ 검증 안 됨 표시) 순서로, 의견서·검토메모에 바로 붙일 수 있는 뼈대가 나옵니다.

### 한눈에 비교

| 그냥 AI에게 물을 때 | 이 MCP를 붙였을 때 |
| --- | --- |
| 해석례 번호를 지어낼 수 있음 | 실존 대조 후 인용 |
| 몇 년도 조문인지 안 따짐 | 귀속연도별 조문 버전 확인 |
| 폐지된 해석을 그대로 인용 | 유효성 판정 후 경고 |
| 국세청 해석·통칙 접근 불가 | 국세청 전용 서고 직접 검색 |
| 계산식 조문을 누락 | 수식 포함 원문 |

> 별도 API 키·비용이 없고, 법제처 MCP(`korean-law-mcp`)와 **짝으로** 쓸 때 완성됩니다 — *조문·판례는 법제처, 해석·통칙·심판례는 국세청.* 최종 판단은 사람이 원문·전문가 확인을 거쳐야 합니다.
