...
리뷰 각도를 바꿔 **불변식(invariant) 검증**과 **부분 장애(상류 한쪽 다운) 거동 검증**을 돌려 발견한 1건. 검증 중 NTS(taxlaw.nts.go.kr)가 실제로 접속 불가 상태가 되어, 평소 재현하기 어려운 장애 경로를 실측할 수 있었다.

### Fixed — verify 헤더가 조회 실패를 '검증 완료'로 표기 (P1)
- **증상**: `verify_nts_citations`의 헤더 줄이 **결과가 나오기 전에** 조립돼 `cits.length`(= 시도 건수)를 썼다. NTS 장애로 3건 전부 조회 실패인데도 **`검출 3건 중 3건 검증.`** 으로 찍혔다.
  - 상세(`? … 조회 실패(NTS 장애 가능) — 실존 판정 불가`)와 요약(`✓ 확인 0 / ✗ 미발견 0 / ? 실패 3`)은 **정확했다**. 즉 v0.21.0의 '장애→미발견 둔갑' P0 수정은 지금도 작동 중이다.
  - 문제는 **헤더만 읽으면 게이트 통과로 오독**된다는 점. 인용 게이트는 산출물 직전 마지막 방어선이라 헤더 오독의 비용이 크다. v0.27.1의 `표시 N개` 불일치와 동형(요약 헤더가 실제와 어긋남).
- **수정**: 헤더를 placeholder로 두고 `confirmed/notFound/failed` 확정 후 교체.
  - 전부 실패: `⚠ 검출 N건 — N건 전부 조회 실패(상류 장애 가능)로 실존 미검증. 이 결과로 인용 게이트를 통과시키지 마라.`
  - 일부 실패: `검출 N건 중 C건 확인 / F건 미발견 / ⚠ X건 조회 실패(미검증).`
  - 실패 0: 종전과 동일.

### Tests
- 338 → **339**. `fetch-deadline.test.js`: fetch를 거부시켜 상류 장애를 모사하고 헤더에 '검증' 표기가 남지 않는지 / '전부 조회 실패'·'게이트 통과 금지' 문구 / 요약 카운트 / **장애가 '미발견'으로 둔갑하지 않는지**(v0.21.0 P0 회귀 방지).
- `npx tsc --noEmit` 통과, `npm test` 339/339 통과.

### 검증 각도 (전부 0건)
- **불변식 7종**: ① 연도 스윕 단조성(year↑인데 시행일 후퇴 금지, 요청 연도 이후 본 선택 금지) ② 적용시점 판단 일관성(`기준 X 이후 개시`면 y<X=종전 / y≥X=개정 — 부등호·결론 뒤집힘) ③ 파라미터 등가성(`year=YYYY` ≡ `efYd=YYYY1231`, MST·본문 동일) ④ 멱등성(동일 호출 2회 결과 일치) ⑤ docType↔dcmClCd 범위(interpretations=01~04 / disputes=05~10 이탈) ⑥ 정렬·페이징(date_desc 실제 내림차순, page1∩page2=∅) ⑦ full 단조성(full=false ≤ full=true).
- **부분 장애 거동**: NTS 불가·법제처 정상 상태에서 — NTS 의존 8종은 전부 `EXTERNAL_API_ERROR` + 가드 + 다음 액션으로 정확히 실패(장애를 '결과 없음'으로 둔갑시키지 않음), **법제처 의존 6종은 전원 정상**(교차 오염 없음: 조문 2.5s·부칙 3.0s·trace 4.0s), **로컬 4종은 1~55ms 정상**(네트워크 무관). 도구 예산도 상한을 정상 적용(최대 90.0s).
- **표기 변형 내성**: 조문 공백(`제29조의 8`)·`제` 생략·법령명 공백·`「」`·약칭(`조특법`) — 조용히 다른 법령/조문을 반환하는 경로 없음.
- **항 완전성**: 항 번호(①②③…) 불연속 시 '삭제' 표기 동반 여부, 절단 안내와 truncate 표기 정합.

### 미검증 (상류 장애로 보류)
- NTS 의존 각도는 접속 복구 후 재실행 필요: **라운드트립 전수**(검색이 출력한 `문서번호:`를 그대로 verify에 넣어 전건 검출되는지 — v0.27.4 수정의 확장 검증), 가운뎃점 변형 검색 내성(`·` U+00B7 / `ㆍ` U+318D / 공백 / 붙임).

### 보류 (사유 유지)
- tools/list `inputSchema` 감량 · `call_taxlaw_extra.name` enum 부재.
- 40행 eflaw 윈도우 밖 시점본은 현행본 폴백 + ⚠.
- A3 시점맞춤 · EF-1 · E4 · F-full · `credit-eligibility.json` 연계표 정확도 재검토(`provisional`).

## [0.27.5] - 2026-08-07

4축 심층 리뷰 루프(적용시점·준용체인·규범계층·업종코드)를 **의미 검증**(구조 검사가 아니라 도구 간 교차 일관성·조문 대조)으로 돌려 발견한 1건. 전 축 0건 수렴 후 릴리스.

### Fixed — 업종코드:KSIC 1:N 매핑의 조용한 누락 (P1)
- **증상**: `findByUpjong`이 `.find()`로 **첫 레코드만** 반환하고 `lookup_upjong_code` 렌더러도 그것만 표시해, **나머지 매핑이 존재한다는 사실조차 알리지 않았다.**
  - 실측: `143107`(조광권자)은 KSIC **5건**(7110 석회석·점토 / 7121 건설용 석재 / 7122 모래·자갈 / 7210 화학용·비료원료 / 7290 그 외 비금속)에 대응하는데 **7110만 노출**.
  - 연계표 전량 검사 결과 **1:N 매핑이 79종**. 업종코드가 여러 KSIC 세세분류에 걸치면 **조특 §6③·§7①1호 목 판정이 KSIC별로 갈릴 수 있어**, 하나만 보고 내린 결론은 틀릴 수 있다. 이 프로젝트가 반복해 고쳐온 '무라벨 누락' 계열.
- **수정**: `findAllByUpjong(code)` 신설(전건 반환). 렌더러는 대표 1건 + `── 복수 매핑(같은 업종코드의 다른 KSIC N건) ──` 목록 + 상단 ⚠ 고지.
- **1:N 판정 기준**: '레코드 수'가 아니라 **'고유 KSIC 코드 수'**. 연계표 원본에 표기 흔들림으로 같은 업종+같은 KSIC가 2행인 케이스가 있다(`630702` 지입: 49301이 "일반 화물 자동차 운송업"/"일반 화물자동차 운송업" 2행 + 49302 = **3행이지만 고유 2건**). 레코드 수로 세면 "KSIC 3건"으로 부풀려진다. siblings 목록도 고유 코드로 dedupe.
- 검증: `630702` → 고지 2건·나열 1건 / `143107` → 고지 5건·나열 4건 / `101000`(1:1) → 무고지.

### Tests
- 333 → **338**. `upjong.test.js`: 1:N 전건 반환(143107의 KSIC 5건) / 1:1 회귀 / 잘못된 입력 빈배열 / 표기 흔들림이 1:N 카운트를 부풀리지 않음(630702 3행↔고유2) / 연계표 순수 중복 0건 무결성.
- `npx tsc --noEmit` 통과, `npm test` 338/338 통과.

### 4축 리뷰 루프 결과 (수렴)
축별 의미 검증기를 새로 만들어 돌렸다. 핵심은 **도구 간 교차 일관성** — 두 도구가 같은 데이터를 다르게 말하면 하나는 틀린 것.
- **축1 적용시점**: 부칙 유형 태깅(과세연도개시/신고시점/최초공제연도/경과조치)이 문언과 정합 / trace↔timetable 개정 인벤토리 일치 / 결박검증 자기모순 / 귀속연도 판단노트 누락 / 공리 블록 부착 / **diff↔시점본 교차검증**(diff가 '변경없음'인데 본문이 다르면 모순). 8+6 시나리오 **0건**.
- **축2 준용체인**: 본문에 '준용'이 있는데 미탐 / 감지 대상이 본문에 실재하지 않는 유령 참조 / 타법 준용 시 그 법령 조문 실제 회수 가능 여부(체인 완주). 6 시나리오 **0건**.
- **축3 규범계층**: 위임 문언 있는데 가드 미탐 / 가드 있는데 문언 없는 오탐 / 시행규칙 위임 시 실제 시행규칙 회수. 6 시나리오 **0건**.
- **축4 업종코드**: 도구 출력↔DB KSIC 일치 / KSIC 역조회 일관성 / 감면 판정↔연계표 일치 / provisional 경고 부착. 109 표본 **0건**(이번 수정 전 1:N 누락만 검출).
- **축4-보강(조문 대조)**: 연계표가 **실존하지 않는 호·목을 참조하는지** 현행 조문에서 직접 추출해 대조 — §6③ 조문 호=1~18 vs 연계표 사용 1~18(9 제외), §7①1호 조문 목=49개 vs 연계표 41개. **전부 조문 범위 내**(범위 이탈 0건).
- **데이터 전량 무결성**(도구 호출 0): 업종 1,784 / 연계표 1,611 / 구조개편 매핑 23. 검출된 3건은 전부 정상 구조로 판별 — 1:N 79종(완전중복 0), KSIC 미부여 1건(381007 "기타 소사장제" = KSIC 대응 분류 없음, note 명시), 연계표 orphan 35건(농 23·임 5·어 7 — §7①1호 가목 대상이나 홈택스 연계표 미수록). orphan 코드 호출 시 판정·비고 정상 출력, 미수록 코드는 `NOT_FOUND` + 다음 액션 안내 확인.
- **구조 루프 재검증**: 11 라운드 × 10 시나리오 **0건**(v0.27.4 수정 회귀 없음).

### 관측 (제품 결함 아님)
- 루프 후반 상류(NTS·법제처) 응답이 크게 지연됐다(동일 라운드 8s → 516s). 도구 시간예산(90s)이 개별 호출을 정상 상한 처리했고 결과 정합성에는 영향 없음.

### 보류 (사유 유지)
- tools/list `inputSchema` 감량 · `call_taxlaw_extra.name` enum 부재.
- 40행 eflaw 윈도우 밖 시점본은 현행본 폴백 + ⚠(조문 신설 이전 연도에 "미존재" 적극 판정 없음).
- A3 시점맞춤 · EF-1 · E4 · F-full · `credit-eligibility.json` 연계표 정확도 재검토(`provisional` — 이번에 호·목 범위 이탈은 없음을 확인).

## [0.27.4] - 2026-08-07

v0.27.3 배포 후 **라이브 리뷰 루프**(11 라운드 × 10 시나리오 = 110건, 연속 2회 무발견까지 반복)에서 누적 수집한 3건. 특이점 0건으로 수렴 후 일괄 릴리스.

### Fixed — 인용 게이트 라운드트립 구멍 (P1)
- **법원 사건번호 하이픈 표기 미추출** `extractNtsCitations`: 법원 패턴이 구분자로 공백만(`\s?`) 허용해 `대법원-2006-두-18652`·`서울행정법원-2018-구합-62461`·`울산지방법원-2019-구합-6417`·`서울고등법원-2018-누-30459`를 침묵 드롭했다.
  - **문제의 핵심**: 이 표기는 다름 아닌 **본 MCP `search_taxlaw_documents`가 `문서번호:`로 출력하는 형식**이다. 검색 결과를 그대로 산출물에 옮기면 `verify_nts_citations`가 **검출 0건 = 경고 없음**으로 통과시켰다. 자기 출력이 자기 검증기를 못 통과하는 라운드트립 실패.
  - 실측: 7개 형식 중 3개만 검출(`조심-2025-중-2511`·`2006두18652`·`2024누50118`), 하이픈 표기 4건 전부 미검출.
  - 조심/국심은 v0.13.x에서 같은 이유로 이미 `[\s-]?`로 고쳤는데 법원 패턴만 남아 있었다 → 동일 수정. 병합사건(`2021두39997, 39998`)·과발동 방지는 회귀 테스트로 고정.

### Fixed — 편의 도구에서 안전 경고가 조용히 사라지던 것 (P1)
- **`research_taxlaw_topic` 재절단** : 이 매크로는 `formatDocumentDetail(...)` 결과를 다시 `truncate(9,000/20,000)` 했다. v0.27.1에서 그 함수 **내부**를 "경고 우선 예산"(`budgetedJoin`)으로 고쳤지만 **결과를 재절단하는 호출부를 놓쳐**, 꼬리의 연도검증·구조개편·통칙 검증이 통째로 잘렸다(실측 3/3에서 연도검증 소실 사례 포함).
  - 도구 설명은 *"연도검증·통칙검증·결론부 가드가 그대로 부착된다"* 고 명시하고 있어 **설명과 실제가 어긋난 상태**였다 — 사용자는 "편의 도구 하나로 충분"이라 믿게 된다.
  - → `formatDocumentDetail`에 `capOverride` 추가. 호출부가 cap을 넘기면 내부 `budgetedJoin`이 경고를 먼저 확보하고 본문을 남은 예산으로 자른다.
- **`isFull` 전제 붕괴 보정**: `detectHoldingTruncation`은 `isFull=true`면 "전문이 있으니 절단 경고 불필요"로 침묵하는데(의도된 설계), 이 매크로는 `isFull: true`를 넘겨놓고 그 뒤 cap으로 실제로 잘랐다. 실측에서 **첨부에 주문이 없는데 결론부 경고도 없는** 구간이 나왔다. → 실제 절단됐을 때만(`[truncated to` 존재) 판례군·해석례군에 조건부 강경고를 덧붙인다(안 잘리면 무경고 = 노이즈 0 유지).

### Fixed — 인용 카운트 라벨 혼동 (P2)
- `assess_doctrine_validity` 채점표가 `인용 조문 추출: 1건`이라 표시하고 바로 아래 목록에 **15개**를 나열했다. 서로 다른 두 개념(`yearCheck.citations` = 관련규정 섹션의 **인용 블록** / `citedArticles` = **조문 참조**)이 같은 이름을 쓴 탓. → `인용 블록(관련규정 섹션)` / `인용 조문 참조 (자동 추출): N건`으로 분리하고 20건 상한 도달 시 라벨 표기.

### Tests
- 331 → **333**. `utils.test.js`: 법원 하이픈 표기 추출 3건(신규 형식·기존 회귀·과발동 방지), `capOverride` 경로 경고 보존 2건(대조군으로 "종전 방식이었다면 잘렸음"을 먼저 입증).
- `npx tsc --noEmit` 통과, `npm test` 333/333 통과.

### 라이브 리뷰 루프 (수렴 확인)
- 11 라운드 × 10 시나리오. 축: 조문 회수(법령·시점 다양) / 부칙·타임테이블 / 검색·문서 / 인용 라운드트립·게이트웨이 / 경계·오류 입력 / 세목 확장(상증·국조·종부·개소·국징) / 준용·위임 집중 / 인용 형식 다양성 / verbose·full·페이징 조합 / 캐시·반복 / 입력 이상치·주입류.
- 탐지기 10종(OC 노출·라벨 반복·표시개수 불일치·상한 초과·무라벨 절단·에러 무가드·무경고 폴백·준용 대상 미표기·위임 오탐·요지 가드 누락) + stdout 오염.
- **연속 2회 특이점 0건으로 수렴.** 중간에 탐지기 오탐 5건을 판별해 교정했다(부칙 예산 조립·검색 목록의 COMPANION 게이트·`isFull` 침묵은 모두 정상 동작).

### 확인된 정상 동작 (수정 불필요)
- `get_law_addenda` 예산 조립: 59,330자(상한 60,000 이내), `표시 42개 == 실제 블록 42`, `⚠ 미표시 171개` 상단 고지 + 꼬리 안내 병존 — v0.27.1 수정 정상.
- 검색 목록에도 `[COMPANION·인용게이트]`로 `요지≠holding` 고지 부착.
- `isFull=true` 시 절단 경고 침묵은 의도된 설계(요지↔주문 모순이 실제 감지될 때만 ⚠).

### 보류 (사유 유지)
- tools/list `inputSchema` 감량 — 도구 설명은 에이전트 행동 지시라 축소 시 가드 안내 누락 위험. `call_taxlaw_extra.name` enum 부재는 별도 큐.
- 40행 eflaw 윈도우 밖 시점본은 현행본 폴백 + ⚠ — 조문 신설 이전 연도에 "그 시점엔 미존재"를 적극 판정하지 않음.
- A3 시점맞춤 · EF-1 · E4 · F-full · `credit-eligibility.json` 연계표 재검토(`provisional`).

## [0.27.3] - 2026-08-07

MCP 재시작 후 **실사용 라이브 검증**(현행/구조문 구분 · 인용조문 하강 · 판례 요지 오인용 3축, 실 MCP 13콜)에서 발견한 오탐 1건 수정. 검증 자체는 기존 가드가 설계대로 작동함을 확인했다.

### Fixed — 구법 표기 경계 오탐 (P1, 라이브 발견)
- **증상**: `AMENDMENT_CLUE_PATTERNS`의 구법 패턴 `/(?:^|[^가-힣])구\s+[가-힣]+법(?:령|률)?|구법|구\s*조문/`에서 교체(`|`)의 사정거리가 **첫 대안에만** 걸려, `구법`·`구조문`이 경계 없이 substring 매칭됐다.
  → 청**구법**인 / 연**구법**인 / 요**구법**령 / 청**구조문** 이 전부 "구법/구조문 표기"로 오탐.
- **영향**: 심판례·판례 본문은 `청구법인`이 수십~수백 회 등장하므로 **사실상 모든 결정례가 '구법 기반'으로 오분류**됐다. 실측(조심-2025-중-2511, 2023 귀속): 현행 조특법 §6 기반 결정인데 `partially_outdated`(부분 사문화 가능성)로 판정되고, 경고문에 `구법/구조문 표기` 라벨이 **62회 반복** 출력됐다. 진짜 구법 기반 문서와 구별이 불가능해져 **사문화 경고가 무력화(crying wolf)** 되는 것이 실질 피해.
- **수정**: 전체 대안을 그룹으로 묶어 앞 경계(문자열 시작 또는 비한글)를 강제 — `/(?:^|[^가-힣])(?:구\s+[가-힣]+법(?:령|률)?|구법|구\s*조문)/`. 정탐(`구 조세특례제한법`·`구법 조문`·`개정 전 구 조문`·`구조문 대비`)은 전부 유지.
- **부수**: 경고문의 단서 라벨 중복 제거(`[...new Set(...)]`) — 같은 라벨이 인용 수만큼 반복되던 노이즈 제거.

### Tests
- 324 → **328**. `year-check.test.js`: `청구법인` 오탐 차단 / `연구법인`·`요구법령`·`청구조문` 오탐 차단 / 진짜 구법 표기 정탐 유지(회귀) / 심판례 본문 형태에서 `partially_outdated` 오분류 소멸.
- `npx tsc --noEmit` 통과, `npm test` 328/328 통과.

### 라이브 검증 결과 (v0.27.0~0.27.2 실사용 확인)
- **현행/구조문 구분**: 40행 윈도우 안이면 시점본 정확 회수(`year=2023` → 시행 2023.12.21 MST 251949) + "현행본에서 본문이 변경됨 — 이 구버전 문구로 결론 단정 금지" + 다음 호출 제시. 윈도우 밖이면 "요청 시점 텍스트 아님"을 명시하고 현행본 반환(무경고 폴백 없음). 법인세법 §25 접대비→기업업무추진비 diff 16 hunk 정확 포착.
- **인용조문 하강**: 시행규칙 본문·수식 이미지 회수, 세법집행기준(번호·수록면·법규성 없음 경고), 타법 준용 자동 해소(조특령 §100의16 → 「법인세법」 §54, 범위 준용 §14~54 라벨 + 타법 MST/URL provenance), 준용 2층 타임라인 분리(§11의2⑧ 자체=신고시점 소급 / §26의8⑥ 준용 경로=2026 이후 개시 → 2025 귀속은 종전규정).
- **v0.27.1 효과 실증**: 준용 컨텍스트 창 120자 확대로 「북한이탈주민…법률」 비인접 케이스에 `lawHint`(귀속 모호) 신호 부착 — 40자였으면 신호 자체가 없었다. `get_law_addenda` 헤더 "표시 N개"가 실제 렌더 수와 일치(213개 중 필터 2 → 표시 2).
- **판례 요지 오인용 방어**: 검색 단계 `relevance_low overflow(5/5건 원본 토큰 미포함 — 사실상 매칭 실패)`·`↻ 자동 재시도` 고지·`생산 15년 경과` 경고, 상세 단계 `요지-결과 정합성`·`목록 메타데이터(결정 칸) 의존 금지` 경고(실측: 메타 `결정 N/A`인데 본문 주문은 `기각`), 인용 단계 명제 결박이 의도적 오귀속을 `⚠⚠ 매칭 20% — 인용 제외` 로 포착. 정탐 인용도 "토큰 존재만 확인, 부정어·극성 미검증"으로 한계 자인. 미수록 대법원 판례는 "공개DB 미발견"까지만.

### 보류 (사유 유지)
- tools/list `inputSchema` 감량 — 도구 설명은 에이전트 행동 지시라 축소 시 동반 호출·가드 안내 누락 위험. `call_taxlaw_extra.name` enum 부재는 별도 큐.
- A3 시점맞춤 · EF-1 · E4 · F-full · `credit-eligibility.json` 연계표 재검토(`provisional`).
- **신규 큐**: 40행 eflaw 윈도우 밖 시점본은 현행본 폴백 + ⚠ 로만 처리 — 조문 신설 이전 연도(예: §29의8, 2021)에 "그 시점엔 미존재"를 적극 판정하지는 않는다.
...
### Added — 하위 위임 감지 가드
- **D1** `buildDelegationGuard(body, jo)` 신설 + `get_law_article`·`build_application_timetable`에 주입. 조회한 조문 본문이 하위규범에 위임하면("…정하여 고시한다/고시로 정한다/장관·청장·위원회가 정하는" → 고시·훈령·예규, "총리령·○○부령으로 정한다" → 시행규칙) 결론(특히 적용시점·판정시점·판정단위·계산방법) 전 그 하위규범을 확인하도록 능동 경고(행정규칙=korean-law search_admin_rule/get_admin_rule, 국세 소관=집행기준·기본통칙 병행). timetable은 기존 '준용 체인' 추적과 같은 자리에서 '위임 체인'도 함께 표기.
  - **근거(실측 사고, 2026-07-14)**: 조특법 §30(중소기업 취업자 소득세 감면)에서 중기령 §3의3**④**가 "…중소벤처기업부장관이 정하여 고시한다"로 적용기간 세부를 고시(「중소기업 범위 및 확인에 관한 규정」)에 위임했는데, 시행령 §3의3① 본문 단서(관계기업)만 보고 "공시집단 편입 즉시전환 규정 없음"으로 단정 → 고시 §3②2호("법 제2조제1항 단서 해당 기업 = 해당 사유가 발생한 날부터 적용")를 놓쳐 결론이 '감면 O'로 뒤집혔던 것을 '감면 X'로 바로잡는 계기. 상위 조문의 위임 화살표를 읽고도 하강하지 않는 반복 실수를 도구 차원에서 차단.
  - **과발동 방지**: '대통령령으로 정한다'(에이전트가 통상 시행령으로 하강)는 제외 — 자주 누락되는 고시·행정규칙과 시행규칙(부령·총리령) 위임만 포착. 위임 신호가 없는 단순 현행조문 조회에는 무발동.

### Tests
- `test/delegation-guard.test.js` 신설(8 케이스): 고시/국세청장/시행규칙 위임 발동 + jo 헤더 + '대통령령'·무위임·빈입력 무발동.
...
### Fixed — 해석 분기 가드(buildInterpretiveForkGuard) 확장
- 신규 패턴: 폐쇄 호구분형 탐지 — `/각\s*호의\s*구분에\s*따른\s*금액/` + `/공제받은\s*금액\s*상당액/`이 함께 있으면 별도 가드 블록 발화. 구 조특법 §30의4②처럼 요건(청년등 감소 등) 판정 후 해당 호에 적힌 '상당액'만 인정되는 조문에, 다른 공제(조특법 §29의7 등)의 단가전환·유추 법리를 준용하지 말라고 경고 + 개정 전후 같은 항 번호가 다른 규정(공제↔사후관리·추징)을 가리킬 수 있으니 시점본(법률 번호) 명기를 강제.
- 기존 사후관리·추징형 가드 양방향화: 기존 '소멸 오독 방지'(단가전환 방향) 메시지 끝에 역방향 경계 문장 추가 — "역방향 오류도 경계: 폐쇄 호구분형 조문에는 이 전환 법리를 유추 적용하지 마라(해석례 부재 시 문언 우선)."
- 배경(실측 2026-07-13): 기존 가드는 "적용하지 아니한다"형 문언에만 발화(정규식 `/적용하지\s*(?:아니|않)/`)해 폐쇄 호구분형 조문에는 침묵했고, 메시지도 관대한 방향(소멸 오독 방지) 단방향이라 반대 방향 오류(다른 공제의 관대한 전환 법리를 폐쇄 호구분 조문에 유추)를 막지 못했다.
- 순수 함수(입력 text·jo → string[]) 성격·기존 반환 형식 유지. 테스트 +1(215 통과): §30의4② 유사 텍스트→신규 가드 / §29의7② 텍스트→기존 가드+양방향 문장 / 무관 조문→미발화.
