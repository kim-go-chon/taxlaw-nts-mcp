...
### Fixed
- 적용연도를 일반 시행일로 대체하지 않고 원문 대조 전 판정을 유보하며, 월·일이 포함된 경과 범위에서 연도 조건을 잘못 배제하지 않도록 보완.
- 신구조문 비교에서 양측 응답의 제명과 요청 법령의 일치 여부를 검증하도록 보완.

## [0.27.9] - 2026-09-10

### Fixed
- 두 조회 본문의 문구 동일을 구간 무개정·부칙 무관으로 단정하지 않도록 설명을 정정. 부칙 조 경계는 개행 시작을 우선하여 본문 괄호 인용이 제목으로 바뀌는 오귀속을 줄임.
- 이 법·영·규칙의 명시 시행일만 추출하며, 미해석·단서·조별 시행일은 미확정으로 두고 귀속연도 판정을 유보. 공포일을 미해석 시행일 대신 사용하지 않음.
- 법령 검색·시행본 선택·부칙 보강에서 정확한 제명 일치가 없는 후보를 제외. 시행령·시행규칙 또는 제명 없는 검색 결과가 법률의 시행본으로 채택되던 오귀속을 차단.
- 법령 본문과 부칙 보강 XML의 실제 제명을 검증하고 요청 제명·주 법령과 불일치하면 오류 처리.
- 과거 시점 조회의 정확 제명 후보가 0건인 경우에도 현행본 대체 시 요청 시점 텍스트가 아님을 명시.
- 회귀 검증: 잘못된 검색 제명·행 래퍼 누락·XML 제명 불일치·정상 부칙 보강·과거 시점 경고를 포함해 381개 테스트 통과.
...
### Fixed
- 인용 실존 검증은 문서번호·회신번호의 전체 식별자만 확정 근거로 사용. 접두 번호 및 제목 속 참조 사건으로 잘못된 문서를 `exists:true`로 기록하던 문제 수정. 법원명 접두 표기는 계속 지원.
- 첫 네트워크 연결 실패부터 재시도를 종료하여 장애 시 최초 호출의 요청 증폭을 4회에서 1회로 축소. HTTP 재시도는 유지.
- korean-law 연계 안내에서 확인되지 않은 연혁 고장 단정을 제거하고, 동일 조문 재사용·MST 인자·현재 통합 도구 경로를 명시.
- `get_execution_standard`의 번호·제목 미조회 경로가 성공 응답처럼 보이던 문제를 `NOT_FOUND` 상태로 명시.
- 보안: `qs`를 6.15.3에서 6.16.0으로 표적 업데이트했으며, `npm audit --omit=dev --json` 취약점 0건을 확인.
...
### Added — 상류 서킷 브레이커 (P1, 실사고 원인)
- **사고 경위**: 검증 루프 중 NTS가 느려지기 시작(동일 라운드 15s → 516s). 그런데 `fetchWithRetryCore`는 호출마다 **최대 4회 재시도**(연결 타임아웃 10s × 4 + 백오프 4.9s)를 계속 던졌다. 느려질수록 트래픽이 4배가 되는 구조다. 결과: `taxlaw.nts.go.kr`·`www.nts.go.kr`·`hometax.go.kr` **전부 IP 차단**(traceroute 7홉부터 드롭, 다른 `.go.kr`는 정상, 타인은 접속 정상). 사용자 체감도 나빴다 — 호출 1건당 **47초** 기다린 뒤 실패(실측).
- **설계**
  - **호스트별 격리** — NTS가 죽어도 법제처 경로는 무영향(부분 장애 검증에서 확인된 격리 특성 유지).
  - **연결 실패만 집계** — HTTP 4xx/5xx는 서버가 응답한 것이므로 건강 신호로 취급하지 않는다(HTTP 500 4회로 회로가 열리지 않음을 테스트로 고정).
  - **점진적 축소** — 연결 실패가 1회라도 쌓이면 그 호스트의 재시도를 0으로 낮춘다(죽은 상류에 4배 부하 금지).
  - **회로 오픈** — 연속 `TAXLAW_CIRCUIT_THRESHOLD`(기본 3)회면 `TAXLAW_CIRCUIT_COOLDOWN_MS`(기본 60초, 지수 증가·상한 10분) 동안 네트워크를 아예 건드리지 않고 즉시 실패.
  - **자동 복구** — 쿨다운 종료 후 half-open 1회 시도, 성공하면 즉시 닫힘.
  - 진단·리셋용 `circuitStatus()` / `resetCircuits()` export.
- **효과(모킹 실측)**: 죽은 상류 5회 연속 호출이 `4932ms → 1ms → 0ms → 회로차단 → 회로차단`. **총 24.7초 → 4.9초(80% 단축), 상류 요청 20회 → 4회(80% 감소).**
- **오류 메시지**: `[UPSTREAM_CIRCUIT_OPEN]` + 남은 차단 시간 + *"'데이터 없음'이 아니라 상류 장애/차단입니다 — 결과를 추측·생성하지 마라"* + 우회 안내(korean-law-mcp) + *"반복 호출은 차단을 연장시킵니다"*. 미존재 오인과 재호출 폭주를 동시에 차단.

### Fixed — 조문 본문의 항·호·목 번호 중복 (P2)
- 법제처 XML은 번호를 **`<항번호>`와 `<항내용>` 양쪽**에 담는다(`<항번호>①</항번호><항내용>① 이 조에서 …</항내용>`). `extractArticleBody`가 모든 CDATA를 이어붙이는 탓에 매 조문 본문이 **`①①`·`1.1.`·`가.가.`·`③③`** 로 찍혔다.
  - 실측(법인세법 MST 280349): **642/642 항 전부 중복**, 조문 255건 본문 175,687자 → **173,301자(1.36% 절감)**.
  - 부작용도 있었다: 항 분할 `split(/(?=[①②③…])/)`이 `"①"` 단독 조각을 하나씩 더 만들어냈다.
  - 가독성 문제도 실재 — `1.1.`은 "1의1"로, `가.가.`는 중복 오타로 오독될 수 있다(검증 중 실제로 필자의 추출 정규식이 이것 때문에 오작동했다).
- **수정**: 순수 번호 청크가 **바로 뒤 청크의 접두일 때만** 생략. 둘이 어긋나면 보존하므로 정보 손실 없음(테스트로 고정).

### Tests
- 343 → **348**. `fetch-deadline.test.js`: 서킷 5종(threshold 도달 시 즉시 차단 / 호스트 격리 / 쿨다운 후 자동 복구 / HTTP 500은 회로 미개방 / 메시지가 '데이터 없음'과 구별되고 추측 금지·재호출 억제를 포함). `utils.test.js`: 마커 중복 4종(항·호·목 제거 / 번호와 본문 불일치 시 보존 / 마커 아닌 짧은 청크 보존).
- `npx tsc --noEmit` 통과, `npm test` 348/348 통과. **NTS를 전혀 호출하지 않고 검증**(fetch 모킹 + 저장해둔 법제처 원본 XML).

### 운영 메모
- 이번 차단은 자동 해제될 때까지 대기가 정석이다. 반복 접속 시도는 차단을 연장시킨다.
- 검증 하네스에도 rate limit·서킷을 넣어야 한다(이번엔 하네스가 상류 상태를 모니터링하지 않고 루프를 반복한 것이 사고의 직접 계기).

### 미검증 (상류 차단으로 보류)
- NTS 의존 각도: 라운드트립 전수(검색 출력 `문서번호:`를 그대로 verify 투입), 가운뎃점 변형 검색 내성. 차단 해제 후 재실행.

### 보류 (사유 유지)
- tools/list `inputSchema` 감량 · `call_taxlaw_extra.name` enum 부재 · 40행 eflaw 윈도우 밖 시점본 폴백 · A3 · EF-1 · E4 · F-full · `credit-eligibility.json` provisional.
